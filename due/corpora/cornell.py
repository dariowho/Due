...
* No **timestamp** information is provided in the dataset. Event timestamps
should be considered fake.
...
def load():
	...
	for conversation in df_conversations.itertuples():
		...
		episode = Episode(starter_agent, invited_agent)
...
def _build_event(line_id, df_lines):
	...
	return Event(Event.Type.Utterance, datetime.now(), line.character_id, line.utterance)
